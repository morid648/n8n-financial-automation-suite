...
**Done:**
- Phase 0 scaffold + Phase 1 specs ([`docs/SCHEMAS.md`](docs/SCHEMAS.md), [`docs/STANDARDS.md`](docs/STANDARDS.md), [`docs/LANGCHAIN_WIRING_CHECKLIST.md`](docs/LANGCHAIN_WIRING_CHECKLIST.md)). 3 blocking decisions: T1.1 = **Supabase**, T1.2 = **classifier-decides**, T1.3 = **dry-run + approval gate**.
- **All 8 workflow JSONs rewritten** (`workflows/*/*.json`) — every structural fix from PRD §3 applied:
  - 3.1 — the 4 RAG clones now differ (per-workflow prompt, schema, tool name, vector namespace)
  - 3.2 / 3.8 — every agent/chain/classifier/extractor has an `ai_languageModel` wired
  - 3.3 — RAG agents get a `Bridge_Agent_Input` Set node on the main path
  - 3.4 — `Context_Memory_Buffer` on `ai_memory` only
  - 3.5 — Macro Thematic Ideation uses `vectorStoreSupabase`
  - 3.6 — Corporate Comms Triage branches by category; only `billing` → Telegram
  - 3.7 — Data Sanitization: classifier → dry-run gate → Telegram approve/deny → delete; read-only vs delete Gmail creds split
  - §4 — `onError` branch + audit-log write on every AI node / ledger write / delete
- Generated + structurally linted by `scratchpad/gen_workflows.py` + `check_workflows.py` (0 issues: valid JSON, no dangling edges, no orphan nodes, all sub-connections present).
- All 8 workflow READMEs updated with real node references + remaining gaps.

**Not done — needs a running n8n instance + live credentials (every JSON has `REPLACE_*` placeholders):**
- Import-testing each JSON in n8n (param shapes for a few nodes — `apiTemplateIo`, telegram `sendAndWait` response field, textClassifier output order — should be re-checked against the target n8n version).
- Phase 2 tests T2.8/T2.9, Phase 5 audits, Phase 6 sample runs + prod credential swap.

**Assumptions still open:** T1.7 (`Gmail_Operations` = ack reply), T1.8 (only `billing` alert-critical), model defaults.
...
- [x] **T0.4** Place each skeleton `.json` export into its `workflows/<name>/` folder with the filename given in the README. — deps: T0.1 · _(user supplied the 8 JSONs; moved into place, then rewritten — see below)_
...
- [x] **T2.1** Define the read-only Gmail credential scope used to fetch flagged mail (no delete scope on the fetch step). — deps: T1.2, T1.13
- [x] **T2.2** Re-wire the graph: `Gmail_Fetch_Flagged → <approval/dry-run step> → Gmail_Delete_Op`; remove the direct fetch→delete edge. — deps: T1.3
- [x] **T2.3** Implement the approval step: Telegram message listing each candidate (sender, subject, date, reason flagged) with explicit approve/deny action that gates `Gmail_Delete_Op`. — deps: T2.2
- [x] **T2.4** Implement the dry-run config flag: when on, write candidates to the audit log and skip `Gmail_Delete_Op` entirely. — deps: T2.2, T1.12
- [x] **T2.5** Add the post-delete audit-log write (what was deleted, when, why, who approved). — deps: T2.2, T1.12
- [x] **T2.6** Add "Continue on Fail" + error branch to any AI/classification node in this workflow. — deps: T1.14
- [x] **T2.7** Update `workflows/data-sanitization-cron/README.md`: trigger, "flagged" definition, safeguard design, dry-run instructions, required creds, known gaps. — deps: T2.2–T2.5
...
- [x] **T3.6** Build one reference workflow JSON demonstrating the corrected RAG wiring (copy-source for the 4 clones). — deps: T3.2–T3.4 · **BLOCKED — needs the skeleton JSON (T0.4).**
...
- [x] **T4A.1** Wire `ai_languageModel` / `ai_tool` / `ai_outputParser` into the agent per the checklist. — deps: T3.2
- [x] **T4A.2** Add the batch loop-back connection on `Batch_Iterator`. — deps: T3.1
- [~] **T4A.3** Create and enforce a read-only PostgreSQL role; point the workflow credential at it. — deps: T1.13
- [x] **T4A.4** Implement the structured JSON verdict output parser using the T1.4 schema. — deps: T1.4, T4A.1
- [x] **T4A.5** Add "Continue on Fail" + error branch to the agent. — deps: T1.14
- [x] **T4A.6** Add audit-log write for any governance action recorded. — deps: T1.12
...
- [x] **T4B.1** Wire `Gemini_Inference_Engine → Quantitative_Data_Parser` via `ai_languageModel`. — deps: T3.5
- [x] **T4B.2** Implement the invoice extraction schema (T1.6) as the parser's structured output. — deps: T1.6, T4B.1
- [x] **T4B.3** Add the pre-`Master_Ledger_Update` validation node using T1.6 rules; route failures to an error branch. — deps: T1.6, T4B.2
- [x] **T4B.4** Implement `Gmail_Operations` per the T1.7 decision. — deps: T1.7
- [x] **T4B.5** Confirm PDF record generation (APITemplate.io) is wired and receives the extracted fields. — deps: T4B.2
- [x] **T4B.6** Add audit-log write on every ledger update. — deps: T1.12
- [~] **T4B.7** Move any inline API keys/connection strings into the n8n credential store. — deps: none
- [x] **T4B.8** Add "Continue on Fail" + error branch to the parser. — deps: T1.14
...
- [x] **T4C.1** Wire `Gemini_Inference_Engine` into both AI nodes (`NLP_Text_Classifier`, `LLM_Execution_Chain`) via `ai_languageModel`. — deps: T3.5
- [x] **T4C.2** Constrain the classifier to the closed T1.8 taxonomy (no free-text categories). — deps: T1.8, T4C.1
...
- [x] **T4C.4** Route non-critical categories to a no-op / log sink. — deps: T4C.3
- [x] **T4C.5** Add "Continue on Fail" + error branch to both AI nodes. — deps: T1.14
...
- [x] **T4D.1** Copy the corrected RAG wiring from the T3.6 reference into this workflow. — deps: T3.6
- [x] **T4D.2** Apply PRD 3.3 bridge node so the webhook payload reaches `NLP_Financial_Agent`. — deps: T3.3, T4D.1
- [x] **T4D.3** Apply PRD 3.4 memory fix (`ai_memory` to agent). — deps: T3.4, T4D.1
- [x] **T4D.4** Write the screening-specific agent prompt + field extraction for T1.5 schema. — deps: T1.5, T4D.2
- [x] **T4D.5** Add the scoring/ranking step producing a ranked shortlist before `Master_Ledger_Update`. — deps: T1.5, T4D.4
- [x] **T4D.6** Wire Cohere embeddings + vector store (insert/query) to the agent via `ai_embedding` / `ai_vectorStore`. — deps: T4D.1
- [x] **T4D.7** Add "Continue on Fail" + error branch; add audit-log write on ledger update. — deps: T1.14, T1.12
- [x] **T4D.8** Apply cost-control knobs (batch size, model). — deps: T1.15
...
- [x] **T4E.1** Copy corrected RAG wiring from T3.6; apply 3.3 bridge and 3.4 memory fix. — deps: T3.6, T3.3, T3.4
- [x] **T4E.2** Write the summarization/distillation prompt + extraction for the T1.9 summary schema. — deps: T1.9, T4E.1
- [x] **T4E.3** Wire embeddings + vector store sub-connections. — deps: T4E.1
- [x] **T4E.4** Add error branch + audit-log write (if it writes to the ledger). — deps: T1.14, T1.12
- [x] **T4E.5** Apply cost-control knobs. — deps: T1.15
...
- [x] **T4F.2** Copy corrected RAG wiring; apply 3.3 bridge and 3.4 memory fix. — deps: T3.6, T3.3, T3.4, T4F.1
- [x] **T4F.3** Write the macro/sector ideation prompt + retrieval contract for the T1.10 schema. — deps: T1.10, T4F.2
- [x] **T4F.4** Wire embeddings + the chosen vector store via sub-connections. — deps: T4F.1, T4F.2
- [x] **T4F.5** Add error branch + audit-log write (if applicable). — deps: T1.14, T1.12
- [x] **T4F.6** Apply cost-control knobs. — deps: T1.15
- [x] **T4F.7** Update workflow README so node types match prose (resolves PRD 3.5 documentation mismatch). — deps: T4F.1–T4F.6
- [~] **T4F.8** End-to-end test against 5–10 sample research queries. — deps: T4F.7

### 4G — Market Sentiment Engine (RAG clone #4)

- [x] **T4G.1** Copy corrected RAG wiring from T3.6; apply 3.3 bridge and 3.4 memory fix. — deps: T3.6, T3.3, T3.4
- [x] **T4G.2** Write the sentiment-classification prompt + extraction for the T1.11 schema. — deps: T1.11, T4G.1
- [x] **T4G.3** Wire Cohere embeddings + Pinecone insert/query via sub-connections. — deps: T4G.1
- [x] **T4G.4** Add error branch + audit-log write (if applicable). — deps: T1.14, T1.12
- [x] **T4G.5** Apply cost-control knobs. — deps: T1.15
